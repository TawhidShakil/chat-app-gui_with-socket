...
class FirstScreen(tk.Tk):
    def __init__(self):
        ...
        print(screen_width)
        print(self.x_co)
        self.y_co =  int((screen_height / 2) - (400 / 2)) - 50
       
        print(self.y_co)
        #self.geometry(f"650x450+{self.x_co}+{self.y_co}")
        ...
        background = background.resize((650, 600), Image.Resampling.LANCZOS)
        background = ImageTk.PhotoImage(background)

        upload_image = Image.open('images/upload_ca.png')
        upload_image = upload_image.resize((25, 25), Image.Resampling.LANCZOS)
        ...
        head.place(relwidth=1, y=20)

        self.profile_label = tk.Label(self.first_frame, bg="grey")
        self.profile_label.place(x=350, y=75, width=150, height=140)


        upload_button = tk.Button(self.first_frame, image=upload_image, compound="left", text="Upload Image",cursor="hand2", font="lucida 12 bold", padx=2, command=self.add_photo)

        upload_button.place(x=345, y=240)
        ...
        submit_button = tk.Button(self.first_frame, text="Connect", font="lucida 12 bold", padx=30, cursor="hand2",
                                  command=self.process_data, bg="#00FF00", relief="solid", bd=2)

        submit_button.place(x=80, y=240)
...
class ChatScreen(tk.Canvas):
    def __init__(self, parent, first_frame, client_socket, clients_connected, user_id):
        ...
        tk.Label(self, text="Group Chat", font="lucida 15 bold", padx=20, fg="green",
                 bg="#b5b3b3", anchor="w", justify="left").place(x=88, y=30, relwidth=1)
        ...
        # 595656
        # d9d5d4
        container.place(x=30, y=120, width=450, height=550)
        ...
        m_frame.pack(pady=0, padx=0,fill="x", expand=True, anchor="e")
